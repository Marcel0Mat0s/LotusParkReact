    ...
    render(){
        ...
        return(
            <li class="list-group-item">{reservation.vaga} - {reservation.dateInit} - {reservation.dateEnd}
                ...
                <button data-bs-toggle="modal" data-bs-target="#exampleModal" type="submit" className="btn btn-success ms-5" onClick={() => this.props.handleStatusChange()}>{ reservation.status===0 ? 'Fechar' : 'Reabrir' }</button>
            </li>
        );
    }
}
